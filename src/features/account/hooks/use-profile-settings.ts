...
export function useProfileSettings() {
  ...
  const validatePassword = () => {
    ...
    if (!parsed.success) {
      next.newPassword = zodFieldErrors(parsed.error)[''] ?? parsed.error.issues[0]?.message ?? 'رمز جدید نامعتبر است.';
    }
  ...
  const changePassword = () => {
    ...
    password.mutate(
      { currentPassword, newPassword },
      {
        ...
        onError: (error) => {
          setPasswordErrors({ currentPassword: getClientErrorMessage(error, 'رمز فعلی صحیح نیست.') });
        },
      },
    );
  };
  ...
  return {
    session, profile, password, name, currentPassword, newPassword,
    passwordErrors, setPasswordErrors, setName, setCurrentPassword, setNewPassword,
    saveProfile, changePassword,
    canSaveProfile: name.trim().length >= MIN_NAME_LENGTH,
    canChangePassword: Boolean(currentPassword) && newPassword.length >= MIN_PASSWORD_LENGTH,
  };
}
