...
export function useResourceManager(kind: AdminResourceKind) {
  ...
  const addItem = () => {
    ...
    if (!result.success) {
      setFormError(result.error.issues[0]?.message ?? 'نام نامعتبر است.');
      return;
    }
    setFormError('');
  ...
  const saveItem = (item: AdminResource) => {
    ...
    if (!result.success) {
      setEditError(result.error.issues[0]?.message ?? 'نام نامعتبر است.');
      return;
    }
    setEditError(null);
    update.mutate(
      { id: item.id, name: result.data, isActive: item.isActive },
